...
Public Class HelperValidation
    ...
    Public Shared Sub MarkFieldInvalidIfEmpty(txtControl As TextBox, defaultLabelText As String)
        Dim labelIndicator As Label = TryCast(txtControl.Tag, Label)
        If labelIndicator IsNot Nothing Then
            If String.IsNullOrWhiteSpace(txtControl.Text) Then
    ...
    ' Mark ComboBox invalid if no selection: show red asterisk and red label text
    Public Shared Sub MarkFieldInvalidIfEmpty(cmbControl As ComboBox, defaultLabelText As String)
        Dim labelIndicator As Label = TryCast(cmbControl.Tag, Label)
        If labelIndicator IsNot Nothing Then
            If cmbControl.SelectedIndex = -1 Then
                labelIndicator.Text = $"{defaultLabelText} *"
                labelIndicator.ForeColor = Color.Red
            Else
                labelIndicator.Text = defaultLabelText
    ...
    ' Mark DateTimePicker invalid if not changed (default today): show red asterisk and red label text
    Public Shared Sub MarkFieldInvalidIfDefault(dtpControl As DateTimePicker, defaultLabelText As String)
        Dim labelIndicator As Label = TryCast(dtpControl.Tag, Label)
        If labelIndicator IsNot Nothing Then
            If dtpControl.Value.Date = Date.Today Then
                labelIndicator.Text = $"{defaultLabelText} *"
                labelIndicator.ForeColor = Color.Red
            Else
                labelIndicator.Text = defaultLabelText
    ...
    Public Shared Sub ShowAsteriskOnMissedFields(fieldControls As TextBox(), labelControls As Label(), labelTexts As String())
        For i As Integer = 0 To fieldControls.Length - 1
            If String.IsNullOrWhiteSpace(fieldControls(i).Text) Then
                labelControls(i).Text = $"{labelTexts(i)} *"
                labelControls(i).ForeColor = Color.Red ' Ensure text turns red
            Else
                labelControls(i).Text = labelTexts(i)
                labelControls(i).ForeColor = SystemColors.ControlText ' Reset to default
            End If
        Next
    End Sub
    ...
    Public Shared Sub ValidateFieldsInRealTime(fieldControls As TextBox(), labelControls As Label(), labelTexts As String())
        For i As Integer = 0 To fieldControls.Length - 1
            labelControls(i).Text = If(String.IsNullOrWhiteSpace(fieldControls(i).Text),
                                     $"{labelTexts(i)} *",
                                     labelTexts(i))
            labelControls(i).ForeColor = If(String.IsNullOrWhiteSpace(fieldControls(i).Text),
                                         Color.Red,
    ...
    Public Shared Sub MarkFieldInvalidCombo(targetControl As ComboBox, defaultLabelText As String)
        Dim labelIndicator As Label = TryCast(targetControl.Tag, Label)
        If labelIndicator IsNot Nothing AndAlso targetControl.SelectedIndex = -1 Then
            labelIndicator.Text = $"{defaultLabelText} *"
            labelIndicator.ForeColor = Color.Red
            labelIndicator.Visible = True
        End If
    ...
    Public Shared Sub MarkFieldInvalidDatePicker(targetControl As DateTimePicker, defaultLabelText As String)
        Dim labelIndicator As Label = TryCast(targetControl.Tag, Label)
        If labelIndicator IsNot Nothing AndAlso targetControl.Value = DateTimePicker.MinimumDateTime Then
            labelIndicator.Text = $"{defaultLabelText} *"
            labelIndicator.ForeColor = Color.Red
            labelIndicator.Visible = True
        End If
    ...
    Public Shared Sub MarkFieldInvalidLabel(targetControl As DateTimePicker, defaultLabelText As String)
        Dim labelIndicator As Label = TryCast(targetControl.Tag, Label)
        If labelIndicator IsNot Nothing AndAlso targetControl.Value = DateTimePicker.MinimumDateTime Then
            labelIndicator.Text = $"{defaultLabelText} *"
            labelIndicator.ForeColor = Color.Red
    ...
    Public Shared Function ValidateCustomerAge(dtpBirthday As DateTimePicker) As Boolean
        ...
        Dim labelIndicator As Label = TryCast(dtpBirthday.Tag, Label)
        If age < 18 Then
            If labelIndicator IsNot Nothing Then
                labelIndicator.Text = $"{labelIndicator.Text.TrimEnd(" "c, "*"c)} *"
                labelIndicator.ForeColor = Color.Red
            End If
            Return False
        Else
